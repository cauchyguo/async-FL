...
class DelayAdaptiveOptimizer:
    ...
    def optimize(self,group_id,selected_clients_idxs):
        ...
        if self.config.get("algorithm","gp") == "gp":
            self.result =  self.optimize_bandwidth_gp(distances, powers, compute_times,self.total_bandwidth,self.data_size)
        elif self.config.get("algorithm","gp") == "binary":
            self.result =  self.optimize_bandwidth_binary(distances, powers, compute_times,self.total_bandwidth,self.data_size)
        else:
            self.result =  self.optimize_bandwidth_simple(distances, powers, compute_times,self.total_bandwidth,self.data_size)
        ...
        return self.result["delays"]  

    def print_result(self,result):
        for i, bw in enumerate(self.result["bandwidths"]):
            # 使用保存的client_id_list来匹配结果
            print(f"客户端ID {self.selected_clients[i]}: {bw:.4f} MHz", end=",")
        
        # 延迟同样可以匹配
        print("\n各客户端延迟:",end="")
        for i, delay in enumerate(self.result["delays"]):
            print(f"客户端ID {self.selected_clients[i]}: {delay:.4f} 秒", end=",")
            
        print(f"\n总带宽使用: {np.sum(self.result['bandwidths']):.4f} MHz , 最大延迟: {self.result['max_delay']:.4f} 秒")
    ...
    def optimize_bandwidth_gp(self, distances_series, powers_series, compute_times_series, total_bandwidth=50, data_size=42.64):
        ...
        try:
            problem.solve(gp=True, solver=cp.ECOS, abstol=1e-4, reltol=1e-4, max_iters=50)  # 使用GP模式求解
    ...
    def optimize_bandwidth_binary(self, distances_series, powers_series, compute_times_series, total_bandwidth=20, data_size=42.64):
        ...
        for _ in range(100):  # 限制迭代次数
            ...
            if delay_max - delay_min < 1e-2:  # 收敛条件
                break
